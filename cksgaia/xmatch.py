...
def read_xmatch_results(fn, mode):
    ...
    df = df[list(namemap.values())]
    df = io.add_prefix(df, gaiadr+'_')
    return df
...
def read_xmatch_gaia2(fn):
    ...
    df = df.rename(columns=namemap)
    df['angdist'] *= 60*60
    cols = list(namemap.values()) + 'steff steff_err1 steff_err2 srad srad_err1 srad_err2 '.split()
    df = df[cols]
    df = io.add_prefix(df, 'gaia2_')
    return df
...
def xmatch_gaia2(df, gaia, key, gaiadr):
    ...
    # count the number of stars within 8 arcsec
    m.index = m.id_kic.values
    g = m.groupby('id_kic',axis=0)
    m[gaiadr+'_gflux_sum'] = g[gaiadr+'_gflux'].sum()
